//
//  main.m
//  parseDisasmKexts_ios10
//
//  Created by huke on 7/8/17.
...
uint64_t VM_OSObjectnew; //OSObject::new //如果以后alloc函数变得更加复杂, 那就有可能需要这个变量的判断辅助

uint64_t VM_IOService; //IOService
uint64_t VM_IOUserClient; //IOUserClient

//当两个csh的handle都运行在程序中时，解析可能会出现问题
csh handle;
...
//获取macho文件的入口,搜集基类信息
void setup_OSMetaClassFunc(void *firstpage);

//分析内核中的ModInit函数,获取各种基本类信息,对后面分析KEXT有帮助
void AnalysisModInitOfKernel(void *kr_bin);

//分析每个内核扩展中的ModInit函数,主要的分析汇编代码的函数
void AnalysisModInitOfKEXT(void *bin, void *bin_kern);

//分析每个内核扩展中的类重写的MetaClass::alloc函数, 找到vtable的位置
uint64_t AnalysisAllocOfClass(void *bin_kern, uint64_t vmaddr, uint64_t fileoff);

//找出内核二进制中所有有效的内核扩展,并且调用函数开始分析(为解析KEXT的始函数)
void FindKEXTsThenAnalysis(void *kr_bin);
...
void *backup_Registers(); //备份所有寄存器内容, 返回一块内存用于待会恢复寄存器内容, 然后记得释放内存
void restore_Registers(void *backup_buf); //恢复所有寄存器内容
...
int main(int argc, const char * argv[]) {
    ...
    uint64_t kr_size = FilegetSize(ker_path);
    void *kr_bin = malloc(kr_size);
    FILE *fp = fopen(ker_path,"ro");
    ...
    setup_OSMetaClassFunc(kr_bin);
    AnalysisModInitOfKernel(kr_bin);
    FindKEXTsThenAnalysis(kr_bin);
...
char *KextGetBundleID(void *kextmacho, void *bin_kern){
    ...
    uint64 __PRELINK_DATA_startvm = machoGetVMAddr(bin_kern, "__PRELINK_DATA", "__data");
    uint64 __PRELINK_DATA_startfo = machoGetFileAddr(bin_kern, "__PRELINK_DATA", "__data");
...
void setup_OSMetaClassFunc(void *firstPage){
    ...
    //VM_OSObjectnew = LINKEDITlookupVMofSymbol("__ZN8OSObjectnwEm");
    
    printf("OSMetaClass::OSMetaClass -> 0x%llx\n", VMOSMetaClass);
    //printf("OSObject::new -> 0x%llx\n", VM_OSObjectnew);
    puts("\n");
}
...
//分析每个内核扩展中的ModInit函数,主要的f分析汇编代码的函数
...
#pragma mark KEXT_DEBUG:STR OP
                    ...
                    //STR指令仍然有些缺陷, 不过程序可以找到所需要的信息而不需要STR指令对数据的膝盖
...
#pragma mark imp:分析每个类的alloc函数寻找vtable位置
uint64_t AnalysisAllocOfClass(void *bin_kern, uint64_t vmaddr, uint64_t fileoff){
...
#pragma mark ALLOC_DEBUG:ORR OP
            ...
            /*#pragma mark ALLOC_DEBUG:BL OP
             if(strstr(insn[j].mnemonic,"bl")){
             int acount = cs_op_count(handle,&insn[j],ARM64_OP_IMM);
             if(acount){
             //过滤: 使用立即数调用函数
             uint64_t bl_func = getSingleIMM(handle, &insn[j]);
             if(bl_func==VM_OSObjectnew){
             x0 = (uint64_t)magicn;
             }
             }
             }*/
...
#pragma mark ALLOC_DEBUG:STR OP
                ...
                int fir_reg = getFirstReg(handle, &insn[j]);
                //nt sec_reg = getSecondReg(handle, &insn[j]);
                ...
                if(possib_vtableaddr){
                    if(*possib_vtableaddr>machoGetVMAddr(bin_kern,"__PLK_DATA_CONST", "__data")){
                        //找到可能的vtable地址, 目前的判断力度足够找到准确的vtable引用位置(或者无法找到vtable返回0)
...
//备份所有寄存器内容, 返回一块内存用于待会恢复寄存器内容, 然后记得释放内存
#pragma mark imp:备份所有寄存器内容, 返回一块内存用于待会恢复寄存器内容, 然后记得释放内存
void *backup_Registers(){
    //目前32个寄存器, 如果增加需要修改这个函数
    void *backup_buf = malloc(regsiters_count*sizeof(x0));
    bzero(backup_buf, regsiters_count*sizeof(x0));
...
//查找指令或者立即数在该类vtable中的引用
#pragma mark imp:查找指令或者立即数在该类vtable中的引用
void findInsReference(char *class_name,uint64_t vtable_start){
    //TO DO ..
}
